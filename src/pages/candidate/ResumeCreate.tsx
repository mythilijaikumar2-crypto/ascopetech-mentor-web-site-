...
export const ResumeCreate: React.FC = () => {
  ...
  const handleSave = () => {
    ...
    const score = Math.floor(Math.random() * 15) + 75; // simulated initial score (75 to 90)
    ...
    <div className="py-8 flex flex-col gap-6 min-h-screen">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-6 border-b border-slate-200 pb-5 no-print">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-xl font-bold text-slate-900 tracking-tight">Interactive Resume Builder</h1>
            <p className="text-[10px] text-slate-400">Fill in form details and preview live template styling.</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 flex flex-col gap-4 no-print">
            ...
            {expandedSection === "objective" && (
              <div className="flex flex-col gap-1.5 pt-3 border-t border-slate-50">
                ...
                <textarea
                  ...
                  placeholder="Detail your professional highlights and career goals..."
                  ...
                  <div key={exp.id} className="flex flex-col gap-4 p-4 border border-slate-100 bg-slate-50/50 rounded-xl relative">
                    ...
                    <div className="flex flex-col gap-1.5">
                      ...
                      <textarea
                        ...
                        placeholder="Explain duties, optimization numbers, systems built..."
            ...
            {expandedSection === "skills" && (
              <div className="flex flex-col gap-4 pt-3 border-t border-slate-50">
                <form onSubmit={handleAddSkill} className="flex gap-2">
                  <Input
                    placeholder="Enter skill (e.g. TypeScript, React)"
                    value={skillInput}
          ...
          <Card className="p-5 border-slate-200 bg-white shadow-sm flex flex-col gap-4">
            ...
            >
              ...
              {expandedSection === "projects" ? <ChevronUp className="h-4.5 w-4.5" /> : <ChevronDown className="h-4.5 w-4.5" />}
            </button>

            {expandedSection === "projects" && (
              <div className="flex flex-col gap-6 pt-3 border-t border-slate-50">
                {projects.map((proj, idx) => (
                  <div key={proj.id} className="flex flex-col gap-4 p-4 border border-slate-100 bg-slate-50/50 rounded-xl relative">
                    <button
                    ...
                    <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wide">Project {idx + 1}</span>
                    <div className="grid grid-cols-2 gap-4">
                      <Input
                        label="Project Title"
                        value={proj.title}
                        onChange={(e) => handleUpdateProj(proj.id, { title: e.target.value })}
                      />
                      <Input
                        label="Project Link / Repository"
                        value={proj.link}
                        onChange={(e) => handleUpdateProj(proj.id, { link: e.target.value })}
                        placeholder="https://github.com/..."
                      />
                    </div>
                    <div className="flex flex-col gap-1.5">
                      <label className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Project Summary</label>
                      <textarea
                        value={proj.description}
                        onChange={(e) => handleUpdateProj(proj.id, { description: e.target.value })}
                        placeholder="Explain architecture patterns, libraries utilized, scope..."
                        className="w-full h-20 p-3 bg-white border border-slate-200 rounded-xl text-xs focus:outline-none focus:border-primary-500"
                      />
                    </div>
                  </div>
                ))}
                <Button variant="outline" size="sm" leftIcon={<Plus className="h-4 w-4" />} className="justify-center py-2" onClick={handleAddProject}>
                  Add Project
                </Button>
              </div>
            )}
          ...
          <Card className="p-5 border-slate-200 bg-white shadow-sm flex flex-col gap-4">
            ...
            >
              ...
              {expandedSection === "certs" ? <ChevronUp className="h-4.5 w-4.5" /> : <ChevronDown className="h-4.5 w-4.5" />}
            </button>
            ...
            {expandedSection === "certs" && (
              <div className="flex flex-col gap-4 pt-3 border-t border-slate-50">
                <form onSubmit={handleAddCert} className="flex gap-2">
                  <Input
                    placeholder="Enter certification (e.g. AWS Cloud Practitioner)"
                    value={certInput}
                ...
                <div className="flex flex-col gap-2 min-h-12 p-3 bg-slate-50 border border-slate-100 rounded-xl">
                  {certifications.length === 0 ? (
                    <span className="text-xs text-slate-400 self-center my-auto">No certifications added.</span>
                  ) : (
                    certifications.map((cert) => (
                      <div key={cert} className="flex justify-between items-center bg-white p-2.5 border border-slate-150 rounded-xl text-xs font-semibold text-slate-750 shadow-sm">
                        <span>{cert}</span>
                        <button type="button" onClick={() => handleRemoveCert(cert)} className="text-slate-400 hover:text-slate-700 font-bold">×</button>
                      </div>
                    ))
                  )}
                </div>
        ...
        {/* Right Side: Sticky Live Preview Container */}
        <div className="lg:col-span-6 lg:sticky lg:top-24 max-h-[85vh] overflow-y-auto bg-white border border-slate-200 p-8 rounded-2xl shadow-lg print-area">
          ...
          {/* Dynamic Template rendering based on selection */}
                    ...
                    {experience.map((exp) => (
                      <div key={exp.id} className="flex flex-col gap-0.8">
                        <div className="flex justify-between items-start font-sans">
                          <strong className="text-slate-800">{exp.role || "Job Role"}</strong>
                          <span className="text-[10px] text-slate-500">{exp.startDate} - {exp.endDate}</span>
                        </div>
                        <p className="text-[10px] font-semibold text-slate-600 font-sans">{exp.company || "Company"}</p>
                    ...
                    {education.map((edu) => (
                      <div key={edu.id} className="flex flex-col gap-0.5 font-sans">
                        <div className="flex justify-between items-start">
                          <strong className="text-slate-800">{edu.degree || "Degree"} in {edu.field || "Field"}</strong>
                          <span className="text-[10px] text-slate-500">{edu.graduationYear}</span>
                        </div>
                        <p className="text-[10px] text-slate-500">{edu.school || "University"}</p>
                      </div>
                    ))}
                  </div>
                )}
              ...
              <div className="flex flex-col gap-6 font-sans">
                ...
                <div className="flex justify-between items-end border-l-4 border-primary-600 pl-4 py-1">
                  ...
                  <div className="text-right text-[10px] text-slate-500 flex flex-col gap-0.5">
                    <span>{personalInfo.email || "email@domain.com"}</span>
                ...
                {objective && (
                  <div className="flex flex-col gap-1.5">
                    <h3 className="text-[11px] font-bold text-primary-650 uppercase tracking-wider border-b border-slate-100 pb-1.5">Executive Summary</h3>
                    <p className="leading-relaxed text-slate-600">{objective}</p>
                  </div>
                )}
                {/* Experience */}
                {experience.length > 0 && (
                  <div className="flex flex-col gap-4">
                    <h3 className="text-[11px] font-bold text-primary-650 uppercase tracking-wider border-b border-slate-100 pb-1.5">Career Progression</h3>
                    {experience.map((exp) => (
                      <div key={exp.id} className="flex gap-4">
                        <span className="text-[9px] font-bold text-slate-400 shrink-0 w-20 text-right mt-0.5">
                          {exp.startDate} - {exp.endDate}
                        </span>
                        <div className="flex flex-col gap-1">
                          <strong className="text-slate-850">{exp.role || "Job Role"}</strong>
                          <span className="text-[10px] text-slate-500 font-semibold">{exp.company || "Company"}</span>
                ...
                {education.length > 0 && (
                  <div className="flex flex-col gap-3">
                    <h3 className="text-[11px] font-bold text-primary-650 uppercase tracking-wider border-b border-slate-100 pb-1.5">Education</h3>
                    {education.map((edu) => (
                ...
                {skills.length > 0 && (
                  <div className="flex flex-col gap-2">
                    <h3 className="text-[11px] font-bold text-primary-650 uppercase tracking-wider border-b border-slate-100 pb-1.5">Skills</h3>
            ...
            {template === "creative" && (
              <div className="grid grid-cols-12 gap-6 font-sans">
                ...
                <div className="col-span-4 bg-slate-900 text-slate-350 p-4 rounded-xl flex flex-col gap-5">
                  <div className="flex flex-col gap-1 text-white border-b border-slate-800 pb-3">
                    <h2 className="text-sm font-bold uppercase tracking-tight">{personalInfo.name || "YOUR NAME"}</h2>
                    <span className="text-[9px] text-slate-500">Applicant Portfolio</span>
                  </div>
                  
                  <div className="flex flex-col gap-3.5 text-[10px]">
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[8px] font-bold text-slate-550 uppercase">Email</span>
                      <span className="truncate">{personalInfo.email || "email@domain.com"}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[8px] font-bold text-slate-550 uppercase">Phone</span>
                      <span>{personalInfo.phone || "Phone"}</span>
                    </div>
                    <div className="flex flex-col gap-0.5">
                      <span className="text-[8px] font-bold text-slate-550 uppercase">Location</span>
                      <span>{personalInfo.location || "Location"}</span>
                    </div>
                  </div>

                  {skills.length > 0 && (
                    <div className="flex flex-col gap-2 border-t border-slate-800 pt-3.5">
                      <span className="text-[8px] font-bold text-slate-550 uppercase">Skills</span>
                ...
                {/* Main details body */}
                <div className="col-span-8 flex flex-col gap-5 pr-2">
                  {objective && (
                    <div className="flex flex-col gap-1">
                  ...
                  {education.length > 0 && (
                    <div className="flex flex-col gap-3">
                      <h3 className="text-[10px] font-bold text-slate-800 uppercase tracking-wider border-b border-slate-100 pb-1">Education</h3>
                      {education.map((edu) => (
                        <div key={edu.id} className="flex flex-col gap-0.5">
                          <div className="flex justify-between items-start">
                            <strong className="text-slate-800 text-[11px]">{edu.degree}</strong>
                            <span className="text-[9px] text-slate-450">{edu.graduationYear}</span>
                          </div>
                          <p className="text-[10px] text-slate-500">{edu.school}</p>
                        </div>
                      ))}
